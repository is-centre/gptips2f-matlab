function [gp_out, fit] = gprule_no_nested_adfs_MT(gp, expr, params)
...
global genCount;
global numGen;
genCountInt=(genCount/numGen)*10;
...
while k <= length(expr)
    ...
    if ~isIntegerValue(genCountInt)&&~isempty(strfind(adfs, expr(k))) %#ok: backwards compatibility
