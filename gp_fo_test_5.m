...
% ADFs
gp.nodes.adf.expr = {'convimp($1, fian(times(?1,$2),0.4))', ...
   'convimp($1, exp(times(?1,$2)))'};

% ADFs for repeated poles
% gp.nodes.adf.expr = {'convimp($1, fian(times(?1,$2),0.3))', ...
%     'convimp($1, convself(fian(times(?1,$2),0.3)))', ...
%     'convimp($1, convself2(fian(times(?1,$2),0.3)))', ...
%     'convimp($1, exp(times(?1,$2)))'};
